...
void token_show(Token * token, FILE * stream) {
    if(token->type == Token_Type_Identifier || token->type == Token_Type_ConstNumber) {
        fprintf(stream, "Token {type = %s, length = %zu, c_str = \"%.*s\"}\n", token_type_str[token->type], token->length, (int) token->length, token->c_str);
    } else {
        fprintf(stream, "Token {type = %s}\n", token_type_str[token->type]);
    }
}
...
Lexer lexer_init(Alloc * alloc, char * filename, FILE * f) {
    ...
    char * buff = new(alloc, sizeof(char) * (size + 1));
...
static Token lexer_read_token(Lexer * self) {
    while(lexer_character(self) != 0) {
        ...
        switch(lexer_character(self)) {
            ...
            case '+':
                return lexer_read_symbol(self, Token_Type_SymbolPlus);
            case '*':
                return lexer_read_symbol(self, Token_Type_SymbolAstrisk);
            case '%':
                return lexer_read_symbol(self, Token_Type_SymbolPercent);
            case '/':
                if(lexer_peek(self) == '/') {
                    ...
                    return lexer_read_symbol(self, Token_Type_SymbolForwadSlash);
                }
            case '0':
            case '1':
            case '2':
...
Token lexer_next_token(Lexer * self) {
    while (self->iterator >= self->token.size) {
        if (self->token.size >= self->token.capacity) {
            ...
            Token * arr = new(self->alloc, sizeof(Token) * self->token.capacity);
            memcpy(arr, self->token.arr, sizeof(Token) * self->token.size);
            delete(self->alloc, self->token.arr);
            self->token.arr = arr;
        }
